...
export function QuestCard({
  ...
  const detailHref = quest.id ? `/quests/${encodeURIComponent(quest.id)}` : undefined
    ...
    <Card className="h-full min-w-0 gap-0 overflow-hidden border-border/70 py-4 shadow-none">
      <CardHeader className="gap-2 space-y-2 px-4 pb-2">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <CardTitle className="line-clamp-2 text-base font-semibold leading-snug">{titleNode}</CardTitle>
      ...
      <CardContent className="space-y-2 px-4 pb-3 pt-0">
        <p className="line-clamp-2 text-sm text-muted-foreground">{quest.description}</p>
        <p className="text-xs text-muted-foreground">
      ...
      <CardFooter className="mt-auto grid grid-cols-2 gap-2 px-4 pt-1">
        <Button
          ...
          className={secondaryAction || onReject ? "w-full" : "col-span-2 w-full"}
