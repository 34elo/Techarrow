...
import { getQuestStatusLabel, type QuestDetail } from "@/entities/quest/model/quest-detail"
...
export function QuestDetailGeneralCard({ quest, className }: QuestDetailGeneralCardProps) {
  ...
  return (
    <Card className={cn("flex h-full min-h-0 flex-col shadow-sm", className)}>
      <CardHeader className="border-b border-border/60 pb-4">
        <CardTitle className="text-base">{t("questDetail.aboutSection")}</CardTitle>
      </CardHeader>
      <CardContent className="flex-1 pt-4">
        <dl>
          ...
          <DetailRow label={t("questDetail.location")}>{quest.location}</DetailRow>
          <DetailRow label={t("questDetail.difficulty")}>{quest.difficulty}</DetailRow>
          <DetailRow label={t("questDetail.duration")}>
            {t("questDetail.durationMinutes", { minutes: quest.duration_minutes })}
          </DetailRow>
          <DetailRow label={t("questDetail.rulesAndWarnings")}>{rules}</DetailRow>
          <DetailRow label={t("questDetail.status")}>{statusLabel}</DetailRow>
          {quest.status === "rejected" ? (
            <DetailRow label={t("questDetail.rejectionReason")}>{rejection}</DetailRow>
          ) : null}
        </dl>
      </CardContent>
    </Card>
