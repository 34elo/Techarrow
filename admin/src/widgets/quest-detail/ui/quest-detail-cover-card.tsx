...
export function QuestDetailCoverCard({ quest, className }: QuestDetailCoverCardProps) {
  const { t } = useTranslations()
  const src = getQuestCoverImageUrl(quest)
  const [showFallback, setShowFallback] = useState(false)
  const alt = quest.title.trim().length > 0 ? quest.title : t("questDetail.cover")
  ...
  return (
    ...
    >
      <CardHeader className="shrink-0 space-y-0 border-b border-border/60 px-4 pb-4">
        <CardTitle className="text-base font-semibold leading-snug">{t("questDetail.cover")}</CardTitle>
      </CardHeader>
      <CardContent className="flex min-h-0 min-w-0 flex-1 flex-col gap-3 px-4 pt-4">
        ...
        {quest.image_file_id ? (
          <p className="break-all text-xs leading-relaxed text-muted-foreground">
            <span className="font-medium text-foreground">{t("questDetail.imageFileId")}: </span>
